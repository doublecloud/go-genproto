...
func (c *logServiceClient) Read(ctx context.Context, in *ReadLogsRequest, opts ...grpc.CallOption) (LogService_ReadClient, error) {
	stream, err := c.cc.NewStream(ctx, &LogService_ServiceDesc.Streams[0], LogService_Read_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &logServiceReadClient{stream}
...
func _LogService_Read_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(LogServiceServer).Read(m, &logServiceReadServer{stream})
}
